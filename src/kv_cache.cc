...
void KVCache::append(const std::string& name, const Tensor& key, const Tensor& value) {
  ...
  cudaMemcpyBytes(new_key.deviceData(), old_key.deviceData(), old_key.nbytes(),
                  CudaMemcpyKind::DeviceToDevice);
  cudaMemcpyBytes(static_cast<char*>(new_key.deviceData()) + old_key.nbytes(), key.deviceData(),
                  key.nbytes(), CudaMemcpyKind::DeviceToDevice);
  cudaMemcpyBytes(new_value.deviceData(), old_value.deviceData(), old_value.nbytes(),
                  CudaMemcpyKind::DeviceToDevice);
  cudaMemcpyBytes(static_cast<char*>(new_value.deviceData()) + old_value.nbytes(),
                  value.deviceData(), value.nbytes(), CudaMemcpyKind::DeviceToDevice);
