...
int main() {
  return dli_test::run("Engine", [] {
    dli::Engine engine;
    dli_test::expect(engine.registry().contains("aten"), "aten dispatcher should be a core builtin");
    dli_test::expect(!engine.registry().contains("conv2d"), "conv2d must not be a CPU builtin");
    dli_test::expect(!engine.registry().contains("attention"), "attention must not be a CPU builtin");
    ...
    engine.registry().registerFactory("identity_test", [] { return std::make_unique<IdentityOp>(); });
