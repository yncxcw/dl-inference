...
@triton.jit
def conv2d_kernel(x, w, bias, out, total, n_dim, h_dim, w_dim, out_c, out_h, out_w,
                  stride_h, stride_w, pad_h, pad_w,
                  in_c: tl.constexpr, k_h: tl.constexpr, k_w: tl.constexpr,
    ...
    for ic in range(0, in_c):
        for kh in range(0, k_h):
            for kw in range(0, k_w):
                ...
                xv = tl.load(x + ((batch * in_c + ic) * h_dim + ih) * w_dim + iw,
                             mask=valid, other=0.0)
                wv = tl.load(w + ((oc * in_c + ic) * k_h + kh) * k_w + kw,
                             mask=mask, other=0.0)
                acc += xv * wv
