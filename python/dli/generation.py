...
class EngineCausalLM:
    """Single-session adapter for one-token DLI causal-LM graphs.

    Prompt tokens are intentionally submitted one at a time because current DLI
    exported decoder graphs have a fixed single-token shape. The engine cache is
    ...
    def _input_device(self) -> torch.device:
        for tensor in self.static_inputs.values():
    ...
    def _run_token(self, token_id: int) -> torch.Tensor:
        if token_id < 0:
        ...
        inputs[self.input_name] = torch.tensor([token_id], dtype=torch.int64, device=self.device)
        run_options: dict[str, Any] = {"position_offset": self._position}
        if self.state is not None:
            run_options["state"] = self.state
        try:
            outputs = self.engine.run(self.graph, inputs, **run_options)
    ...
    def prefill(self, input_ids: Sequence[int]) -> ModelStep:
        ...
        for token_id in prompt:
            logits = self._run_token(token_id)
    ...
    def decode(self, token_id: int, state: Any) -> ModelStep:
        ...
        logits = self._run_token(int(token_id))
        return ModelStep(
            logits,
            EngineGenerationState(self._generation, self._position, self._state_owner),
        )
